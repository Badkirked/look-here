...
globalThis.captureDB = {
  ...
  async prune() {
    ...
    const sorted = all.sort((a,b) => Date.parse(b.capturedAt)-Date.parse(a.capturedAt));
    for (const [index, item] of sorted.entries()) {
      if (index >= 10 || !Number.isFinite(Date.parse(item.capturedAt)) || Date.parse(item.capturedAt) < cutoff) await this.remove(item.id);
    }
  },
